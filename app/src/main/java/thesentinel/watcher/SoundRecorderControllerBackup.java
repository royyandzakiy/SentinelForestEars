...
public class SoundRecorderControllerBackup {
    private MediaRecorder mRecorder;
    private SoundRecorderActivity activity;
    private Thread recordingThread;

    public SoundRecorderControllerBackup(SoundRecorderActivity activity) {
    ...
    public void getPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            activity.requestPermissions(
                    new String[]{Manifest.permission.RECORD_AUDIO}, 13);
        }
    }

    public boolean checkRecordingPermission()
    {
        int result  = activity.getApplicationContext().checkCallingOrSelfPermission(Manifest.permission.RECORD_AUDIO);
